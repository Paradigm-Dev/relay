...
router.post("/create/:id/files/:type", async (req, res) => {
  ...
  const form = formidable({
    ...
    uploadDir:
      __dirname +
      `/../files/${
        req.params.type == "music" ? "music" : req.params.type + "s"
      }/`,
    keepExtensions: true,
  });
  ...
  await form.parse(req, async (err, fields, files) => {
    ...
    if (req.params.type != "music")
      await fs.renameSync(
        files.file.path,
        __dirname +
          `/../files/${req.params.type + "s"}/${
            Item._id
          }.${files.file.name.slice(files.file.name.lastIndexOf(".") + 1)}`
      );
    else {
      await fs.mkdirSync(__dirname + `/../files/music/${Item._id}`);
      await Item.songs.forEach(async (song) => {
        song.file = `https://www.theparadigmdev.com/relay/music/${Item._id}/${
          song.track
        }.${files[song.track].name.slice(
          files[song.track].name.lastIndexOf(".") + 1
        )}`;
        await fs.renameSync(
          files[song.track].path,
          __dirname +
            `/../files/music/${Item._id}/${song.track}.${files[
              song.track
