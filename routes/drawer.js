...
router.post("/:uid/rename/:id", async (req, res) => {
  ...
  if (File.type == "workshop/write") newPath = req.body.name + ".write.json";
    ...
    newPath =
      req.body.name + "." + File.path.slice(File.path.lastIndexOf(".") + 1);
  fs.rename(
    "/mnt/drawer/" + req.params.uid + "/" + File.path,
...
router.delete("/:uid/delete/:id", async (req, res) => {
  ...
  var File = await User.files.id(req.params.id);
  fs.unlink(
    path.join("/mnt/drawer/" + req.params.uid + "/" + File.path),
    async (error) => {
      await File.remove();
      await User.save();
      res.json(User);
...
router.post("/:uid/upload", async (req, res) => {
  const form = formidable({
    multiples: true,
    uploadDir: `/mnt/drawer/${req.params.uid}`,
    keepExtensions: true,
