...
router.get("/chatroom/:id/ban/:user", async (req, res) => {
  ...
  if (Requested_I >= 0)
    await axios.get(
      `https://www.theparadigmdev.com/api/flamechat/chatroom/${req.params.id}/request/${User._id}/reject`
    );
  if (Approved_I >= 0)
    await axios.get(
      `https://www.theparadigmdev.com/api/flamechat/chatroom/${req.params.id}/remove/${User._id}`
    );
